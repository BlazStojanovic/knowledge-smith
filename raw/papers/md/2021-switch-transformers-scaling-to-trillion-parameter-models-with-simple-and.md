...
\emailliamfedus@google.com
\AND\nameBarret Zoph\*
  
\emailbarretzoph@google.com
\AND\nameNoam Shazeer
  
\emailnoam@google.com
  
\addrGoogle, Mountain View, CA 94043, USA
Equal contribution.
...
We design models based off T5-Base and T5-Large (Raffel et al., [2019](#bib.bib36)) to obtain up to 7x increases in pre-training speed with the same computational resources.
...
Finally, we advance the current scale of language models by pre-training up to trillion parameter models on the “Colossal Clean Crawled Corpus”, and achieve a 4x speedup over the T5-XXL model.111JAX code for Switch Transformer and all model checkpoints are available at <https://github.com/google-research/t5x>222Tensorflow code for Switch Transformer is available at <https://github.com/tensorflow/mesh/blob/master/mesh_tensorflow/transformer/moe.py>
...
1. [1 Introduction](#S1 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
2. [2 Switch Transformer](#S2 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   1. [2.1 Simplifying Sparse Routing](#S2.SS1 "In 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   2. [2.2 Efficient Sparse Routing](#S2.SS2 "In 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   3. [2.3 Putting It All Together: The Switch Transformer](#S2.SS3 "In 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   4. [2.4 Improved Training and Fine-Tuning Techniques](#S2.SS4 "In 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
3. [3 Scaling Properties](#S3 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   1. [3.1 Scaling Results on a Step-Basis](#S3.SS1 "In 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   2. [3.2 Scaling Results on a Time-Basis](#S3.SS2 "In 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   3. [3.3 Scaling Versus a Larger Dense Model](#S3.SS3 "In 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
4. [4 Downstream Results](#S4 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   1. [4.1 Fine-Tuning](#S4.SS1 "In 4 Downstream Results ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   2. [4.2 Distillation](#S4.SS2 "In 4 Downstream Results ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   3. [4.3 Multilingual Learning](#S4.SS3 "In 4 Downstream Results ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
5. [5 Designing Models with Data, Model, and Expert-Parallelism](#S5 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   1. [5.1 Data Parallelism](#S5.SS1 "In 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   2. [5.2 Model Parallelism](#S5.SS2 "In 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   3. [5.3 Model and Data Parallelism](#S5.SS3 "In 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   4. [5.4 Expert and Data Parallelism](#S5.SS4 "In 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   5. [5.5 Expert, Model and Data Parallelism](#S5.SS5 "In 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
   6. [5.6 Towards Trillion Parameter Models](#S5.SS6 "In 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
6. [6 Related Work](#S6 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
7. [7 Discussion](#S7 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
8. [8 Future Work](#S8 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
9. [9 Conclusion](#S9 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
10. [A Switch for Attention](#A1 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
11. [B Preventing Token Dropping with *No-Token-Left-Behind*](#A2 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
12. [C Encouraging Exploration Across Experts](#A3 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
13. [D Switch Transformers in Lower Compute Regimes](#A4 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
14. [E Relation of Upstream to Downstream Model Performance](#A5 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")
15. [F Pseudo Code for Switch Transformers](#A6 "In Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")

## 1 Introduction

Large scale training has been an effective path towards flexible and powerful neural language models (Radford et al., [2018](#bib.bib35); Kaplan et al., [2020](#bib.bib25); Brown et al., [2020](#bib.bib4)).
Simple architectures—backed by a generous computational budget, data set size and parameter count—surpass more complicated algorithms (Sutton, [2019](#bib.bib52)).
An approach followed in Radford et al. ([2018](#bib.bib35)); Raffel et al. ([2019](#bib.bib36)); Brown et al. ([2020](#bib.bib4)) expands the model size of a densely-activated Transformer (Vaswani et al., [2017](#bib.bib55)).
While effective, it is also extremely computationally intensive (Strubell et al., [2019](#bib.bib50)).
...
Sparse training is an active area of research and engineering (Gray et al., [2017](#bib.bib14); Gale et al., [2020](#bib.bib13)), but as of today, machine learning libraries and hardware accelerators still cater to dense matrix multiplications.
To have an efficient sparse algorithm, we start with the Mixture-of-Expert (MoE) paradigm (Jacobs et al., [1991](#bib.bib22); Jordan and Jacobs, [1994](#bib.bib23); Shazeer et al., [2017](#bib.bib46)), and simplify it to yield training stability and computational benefits.
MoE models have had notable successes in machine translation (Shazeer et al., [2017](#bib.bib46), [2018](#bib.bib47); Lepikhin et al., [2020](#bib.bib30)), however, widespread adoption is hindered by complexity, communication costs, and training instabilities.
...
Further, our large sparse models can be distilled (Hinton et al., [2015](#bib.bib18)) into small dense versions while preserving 30% of the sparse model quality gain.
...
* •
  ...
  Scaling properties and a benchmark against the strongly tuned T5 model (Raffel et al., [2019](#bib.bib36)) where we measure 7x+ pre-training speedups while still using the same FLOPS per token. We further show the improvements hold even with limited computational resources, using as few as two experts.
* •
...
* •
  ...
  A measurement of the pre-training benefits on multilingual data where we find a universal improvement across all 101 languages and with 91% of languages benefiting from 4x+ speedups over the mT5 baseline (Xue et al., [2020](#bib.bib59)).
* •
...
The guiding design principle for Switch Transformers is to maximize the parameter count of a Transformer model (Vaswani et al., [2017](#bib.bib55)) in a simple and computationally efficient way.
The benefit of scale was exhaustively studied in Kaplan et al. ([2020](#bib.bib25)) which uncovered power-law scaling with model size, data set size and computational budget.
...
Heeding these results, we investigate a fourth axis: increase the *parameter count* while keeping the floating point operations (FLOPs) per example constant.
...
Shazeer et al. ([2017](#bib.bib46)) proposed a natural language Mixture-of-Experts (MoE) layer which takes as an input a token representation x𝑥x and then routes this to the best determined top-k𝑘k experts, selected from a set {Ei​(x)}i=1Nsuperscriptsubscriptsubscript𝐸𝑖𝑥𝑖1𝑁\{E\_{i}(x)\}\_{i=1}^{N} of N𝑁N experts.
...
Shazeer et al. ([2017](#bib.bib46)) conjectured that routing to k>1𝑘1k>1 experts was necessary in order to have non-trivial gradients to the routing functions.
The authors intuited that learning to route would not work without the ability to compare at least two experts.
Ramachandran and Le ([2018](#bib.bib39)) went further to study the top-k𝑘k decision and found that higher k𝑘k-values in lower layers in the model were important for models with many routing layers.
Contrary to these ideas, we instead use a simplified strategy where we route to only a *single* expert.
...
Note that for both MoE and Switch Routing, the gate value pi​(x)subscript𝑝𝑖𝑥p\_{i}(x) in Equation [2](#S2.E2 "In 2.1 Simplifying Sparse Routing ‣ 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") permits differentiability of the router.
...
We use Mesh-Tensorflow (MTF) (Shazeer et al., [2018](#bib.bib47)) which is a library, with similar semantics and API to Tensorflow (Abadi et al., [2016](#bib.bib1)) that facilitates efficient distributed data and model parallel architectures.
...
All of our tensor shapes are statically determined at compilation time, but our computation is *dynamic* due to the routing decisions at training and inference.
Because of this, one important technical consideration is how to set the *expert capacity*.
The expert capacity—the number of tokens each expert computes—is set by evenly dividing the number of tokens in the batch across the number of experts, and then further expanding by a *capacity factor*,
...
This trade-off is explained in Figure [3](#S2.F3 "Figure 3 ‣ 2.1 Simplifying Sparse Routing ‣ 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").
...
We study the impact that these design decisions have on model quality and speed in Table [1](#S2.T1 "Table 1 ‣ 2.3 Putting It All Together: The Switch Transformer ‣ 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").

A Differentiable Load Balancing Loss.
To encourage a balanced load across experts we add an auxiliary loss (Shazeer et al., [2017](#bib.bib46), [2018](#bib.bib47); Lepikhin et al., [2020](#bib.bib30)).
As in Shazeer et al. ([2018](#bib.bib47)); Lepikhin et al. ([2020](#bib.bib30)), Switch Transformers simplifies the original design in Shazeer et al. ([2017](#bib.bib46)) which had separate load-balancing and importance-weighting losses.
...
222A potential source of confusion: pi​(x)subscript𝑝𝑖𝑥p\_{i}(x) is the probability of routing token x𝑥x to expert i𝑖i. Pisubscript𝑃𝑖P\_{i} is the probability fraction to expert i𝑖i across *all tokens* in the batch ℬℬ\mathcal{B}.
...
Our first test of the Switch Transformer starts with pre-training on the “Colossal Clean Crawled Corpus” (C4), introduced in (Raffel et al., [2019](#bib.bib36)).
For our pre-training objective, we use a masked language modeling task (Taylor, [1953](#bib.bib54); Fedus et al., [2018](#bib.bib12); Devlin et al., [2018](#bib.bib9)) where the model is trained to predict missing tokens.
In our pre-training setting, as determined in Raffel et al. ([2019](#bib.bib36)) to be optimal, we drop out 15% of tokens and then replace the masked sequence with a single sentinel token.
...
A comparison of all the models studied in this work are in Table [9](#S5.T9 "Table 9 ‣ 5.6 Towards Trillion Parameter Models ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").
...
A head-to-head comparison of the Switch Transformer and the MoE Transformer is presented in Table [1](#S2.T1 "Table 1 ‣ 2.3 Putting It All Together: The Switch Transformer ‣ 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").
Our Switch Transformer model is FLOP-matched to ‘T5-Base’ (Raffel et al., [2019](#bib.bib36)) (same amount of computation per token is applied).
...
We highlight three key findings from Table [1](#S2.T1 "Table 1 ‣ 2.3 Putting It All Together: The Switch Transformer ‣ 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity"): (1) Switch Transformers outperform both carefully tuned dense models and MoE Transformers on a speed-quality basis.
...
Further, low precision formats like bfloat16 (Wang and Kanwar, [2019](#bib.bib58)) can exacerbate issues in the softmax computation for our router.
...
Model instability hinders the ability to train using efficient bfloat16 precision, and as a result, Lepikhin et al. ([2020](#bib.bib30)) trains with float32 precision throughout their MoE Transformer.
...
Table [2](#S2.T2 "Table 2 ‣ 2.4 Improved Training and Fine-Tuning Techniques ‣ 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") shows that our approach permits nearly equal speed to bfloat16 training while conferring the training stability of float32.
...
Table [3](#S2.T3 "Table 3 ‣ 2.4 Improved Training and Fine-Tuning Techniques ‣ 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") measures the improvement of the model quality and reduction of the variance early in training.
...
During fine-tuning of standard Transformers, Raffel et al. ([2019](#bib.bib36)) use dropout (Srivastava et al., [2014](#bib.bib49)) at each layer to prevent overfitting.
...
Table [4](#S2.T4 "Table 4 ‣ 2.4 Improved Training and Fine-Tuning Techniques ‣ 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") has the results for our expert dropout protocol.
...
We present a study of the *scaling properties* of the Switch Transformer architecture during pre-training.
Per Kaplan et al. ([2020](#bib.bib25)), we consider a regime where the model is not bottlenecked by either the computational budget or amount of data.
To avoid the data bottleneck, we use the large C4 corpus with over 180B target tokens (Raffel et al., [2019](#bib.bib36)) and we train until diminishing returns are observed.
...
Figure [4](#S3.F4 "Figure 4 ‣ 3.1 Scaling Results on a Step-Basis ‣ 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") demonstrates consistent scaling benefits with the number of experts when training all models for a fixed number of steps.
...
Figure [4](#S3.F4 "Figure 4 ‣ 3.1 Scaling Results on a Step-Basis ‣ 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") demonstrates that on a step basis, as we increase the number of experts, the performance consistently improves.
...
Figures [5](#S3.F5 "Figure 5 ‣ 3.2 Scaling Results on a Time-Basis ‣ 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") and [6](#S3.F6 "Figure 6 ‣ 3.3 Scaling Versus a Larger Dense Model ‣ 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") address this question.
Figure [5](#S3.F5 "Figure 5 ‣ 3.2 Scaling Results on a Time-Basis ‣ 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") measures the pre-training model quality as a function of time.
For a fixed training duration and computational budget, Switch Transformers yield a substantial speed-up.
In this setting, our Switch-Base 64 expert model trains in *one-seventh* the time that it would take the T5-Base to get similar perplexity.
...
Figure [6](#S3.F6 "Figure 6 ‣ 3.3 Scaling Versus a Larger Dense Model ‣ 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") considers a different scenario: what if we instead had allocated our resources to a larger dense model?
We do so now, measuring Switch-Base against the next strong baseline, *T5-Large*.
...
Section [3](#S3 "3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") demonstrated the superior scaling properties while pre-training, but we now validate that these gains translate to improved language learning abilities on downstream tasks.
...
Our baselines are the highly-tuned 223M parameter T5-Base model and the 739M parameter T5-Large model (Raffel et al., [2019](#bib.bib36)).
For both versions, we design a FLOP-matched Switch Transformer, with many more parameters, which is summarized in Table [9](#S5.T9 "Table 9 ‣ 5.6 Towards Trillion Parameter Models ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").777FLOPS are calculated for the forward pass as done in Kaplan et al. ([2020](#bib.bib25)).
Our baselines differ slightly from those in Raffel et al. ([2019](#bib.bib36)) because we pre-train on an improved C4 corpus which removes intra-example text duplication and thus increases the efficacy as a pre-training task Lee et al. ([2021](#bib.bib29)).
...
We then fine-tune across a diverse set of tasks using a dropout rate of 0.1 for all layers except the Switch layers, which use a dropout rate of 0.4 (see Table [4](#S2.T4 "Table 4 ‣ 2.4 Improved Training and Fine-Tuning Techniques ‣ 2 Switch Transformer ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")).
...
The language benchmarks GLUE (Wang et al., [2018](#bib.bib56)) and SuperGLUE (Wang et al., [2019](#bib.bib57)) are handled as composite mixtures with all the tasks blended in proportion to the amount of tokens present in each.
...
The CNNDM (Hermann et al., [2015](#bib.bib17)) and BBC XSum (Narayan et al., [2018](#bib.bib32)) data sets are used to measure the ability to summarize articles.
Question answering is probed with the SQuAD data set (Rajpurkar et al., [2016](#bib.bib38)) and the ARC Reasoning Challenge (Clark et al., [2018](#bib.bib7)).
And as in Roberts et al. ([2020](#bib.bib41)), we evaluate the knowledge of our models by fine-tuning on three closed-book question answering data sets: Natural Questions (Kwiatkowski et al., [2019](#bib.bib27)), Web Questions (Berant et al., [2013](#bib.bib3)) and Trivia QA (Joshi et al., [2017](#bib.bib24)).
Closed-book refers to questions posed with no supplemental reference or context material.
To gauge the model’s common sense reasoning we evaluate it on the Winogrande Schema Challenge (Sakaguchi et al., [2020](#bib.bib43)).
And finally, we test our model’s natural language inference capabilities on the Adversarial NLI Benchmark (Nie et al., [2019](#bib.bib33)).
...
In SQuAD and the closed book tasks (Web, Natural, and Trivia Questions) we report the percentage of answers exactly matching the target (refer to Roberts et al. ([2020](#bib.bib41)) for further details and deficiency of this measure).
...
To alleviate this, we study distilling (Hinton et al., [2015](#bib.bib18)) large sparse models into small dense models.
Future work could additionally study distilling large models into smaller *sparse* models.

Distillation techniques.
In Table [6](#S4.T6 "Table 6 ‣ 4.2 Distillation ‣ 4 Downstream Results ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") we study a variety of distillation techniques.
These techniques are built off of  Sanh et al. ([2019](#bib.bib44)), who study distillation methods for BERT models.
...
Using our best distillation technique described in Table [6](#S4.T6 "Table 6 ‣ 4.2 Distillation ‣ 4 Downstream Results ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity"), we distill a wide variety of sparse models into dense models.
...
Table  [8](#S4.T8 "Table 8 ‣ 4.2 Distillation ‣ 4 Downstream Results ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") shows results of distilling a 7.4B parameter Switch-Base model, fine-tuned on the SuperGLUE task, into the 223M T5-Base.
...
In Figure [8](#S4.F8 "Figure 8 ‣ 4.3 Multilingual Learning ‣ 4 Downstream Results ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity"), we present a different view and now histogram the per step *speed-up* of using Switch Transformer over the mT5-Base.999The speedup on a step basis is computed as the ratio of the number of steps for the baseline divided by the number of steps required by our model to reach that same quality.
...
Arbitrarily increasing the number of experts is subject to diminishing returns (Figure [4](#S3.F4 "Figure 4 ‣ 3.1 Scaling Results on a Step-Basis ‣ 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")).
Here we describe *complementary* scaling strategies.
...
We use the FFN layer as an example of how data, model and expert-parallelism works in Mesh TensorFlow (Shazeer et al., [2018](#bib.bib47)) and review it briefly here.
...
We describe two aspects of partitioning: how the *weights* and *batches of data* divide over cores, depicted in Figure [9](#S5.F9 "Figure 9 ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").
...
This corresponds to the left-most column of Figure [9](#S5.F9 "Figure 9 ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").
...
It is common to mix both model and data parallelism for large scale models, which was done in the largest T5 models  (Raffel et al., [2019](#bib.bib36); Xue et al., [2020](#bib.bib59)) and in GPT-3  (Brown et al., [2020](#bib.bib4)).
...
|  |  |  |  |
| --- | --- | --- | --- |
|  | einsum​([n,B/n,dm​o​d​e​l],[n,B/n,E,C],dimension=[B/n])einsum    𝑛𝐵𝑛subscript𝑑𝑚𝑜𝑑𝑒𝑙  𝑛𝐵𝑛𝐸𝐶 dimension delimited-[]𝐵𝑛\text{einsum}([n,B/n,d\_{model}],[n,B/n,E,C],\text{dimension}=[B/n]) |  | (7) |
...
See Appendix [F](#A6 "Appendix F Pseudo Code for Switch Transformers ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") for a detailed analysis of the expert partitioning code.
...
See our further analysis in section [5.6](#S5.SS6 "5.6 Towards Trillion Parameter Models ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") for how the number of experts effects the downstream performance as well.
...
The Switch-C model is designed using only expert-parallelism, and no model-parallelism, as described earlier in Section [5.4](#S5.SS4 "5.4 Expert and Data Parallelism ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").
...
In contrast, the Switch-XXL is FLOP-matched to the T5-XXL model, which allows for larger dimensions of the hyper-parameters, but at the expense of additional communication costs induced by model-parallelism (see Section [5.5](#S5.SS5 "5.5 Expert, Model and Data Parallelism ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") for more details).

Sample efficiency versus T5-XXL. In the final two columns of Table [9](#S5.T9 "Table 9 ‣ 5.6 Towards Trillion Parameter Models ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") we record the negative log perplexity on the C4 corpus after 250k and 500k steps, respectively.
...
To contextualize the significance of a gap of 0.061, we note that the T5-XXL model had to train for an *additional* 250k steps to increase 0.052.
...
As a result, though this is our better model on a step-basis, we do not pre-train for a full 1M steps, in-line with the final reported results of T5 (Raffel et al., [2019](#bib.bib36)).
...
Next, the average SuperGLUE test score is recorded at 87.5 versus the T5 version obtaining a score of 89.3 compared to the state-of-the-art of 90.0 (Wang et al., [2019](#bib.bib57)).
On ANLI (Nie et al., [2019](#bib.bib33)), Switch XXL improves over the prior state-of-the-art to get a 65.7 accuracy versus the prior best of 49.4 (Yang et al., [2020](#bib.bib60)).
...
We study this issue more in Appendix [E](#A5 "Appendix E Relation of Upstream to Downstream Model Performance ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").

Knowledge-based fine-tuning performance. Finally, we also conduct an early examination of the model’s knowledge with three closed-book knowledge-based tasks: Natural Questions, WebQuestions and TriviaQA, without additional pre-training using Salient Span Masking (Guu et al., [2020](#bib.bib15)).
...
[E](#A5 "Appendix E Relation of Upstream to Downstream Model Performance ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")).
...
Recent works have scaled models to billions of parameters through using model parallelism (e.g. splitting weights and tensors across multiple cores) (Shazeer et al., [2018](#bib.bib47); Rajbhandari et al., [2019](#bib.bib37); Raffel et al., [2019](#bib.bib36); Brown et al., [2020](#bib.bib4); Shoeybi et al., [2019](#bib.bib48)).
Alternatively, Harlap et al. ([2018](#bib.bib16)); Huang et al. ([2019](#bib.bib21)) propose using pipeline based model parallelism, where different layers are split across devices and micro-batches are *pipelined* to the different layers.
Finally, Product Key networks (Lample et al., [2019](#bib.bib28)) were proposed to scale up the capacity of neural networks by doing a lookup for learnable embeddings based on the incoming token representations to a given layer.
...
Cho and Bengio ([2014](#bib.bib6)) proposed adaptively selecting weights based on certain bit patterns occuring in the model hidden-states.
Eigen et al. ([2013](#bib.bib10)) built stacked expert layers with dense matrix multiplications and ReLU activations and showed promising results on jittered MNIST and monotone speech.
In computer vision Puigcerver et al. ([2020](#bib.bib34)) manually route tokens based on semantic classes during upstream pre-training and then select the relevant experts to be used according to the downstream task.

Mixture of Experts (MoE), in the context of modern deep learning architectures, was proven effective in Shazeer et al. ([2017](#bib.bib46)).
That work added an MoE layer which was stacked between LSTM (Hochreiter and Schmidhuber, [1997](#bib.bib19)) layers, and tokens were separately routed to combinations of experts.
This resulted in state-of-the-art results in language modeling and machine translation benchmarks.
The MoE layer was reintroduced into the Transformer architecture by the Mesh Tensorflow library (Shazeer et al., [2018](#bib.bib47)) where MoE layers were introduced as a substitute of the FFN layers, however, there were no accompanying NLP results.
More recently, through advances in machine learning infrastructure, GShard (Lepikhin et al., [2020](#bib.bib30)), which extended the XLA compiler, used the MoE Transformer to dramatically improve machine translation across 100 languages.
Finally Fan et al. ([2021](#bib.bib11)) chooses a different deterministic MoE strategy to split the model parameters into non-overlapping groups of languages.

Sparsity along the sequence length dimension (L𝐿L) in the Transformer *attention patterns* has been a successful technique to reduce the attention complexity from O​(L2)𝑂superscript𝐿2O(L^{2}) (Child et al., [2019](#bib.bib5); Correia et al., [2019](#bib.bib8); Sukhbaatar et al., [2019](#bib.bib51); Kitaev et al., [2020](#bib.bib26); Zaheer et al., [2020](#bib.bib61); Beltagy et al., [2020](#bib.bib2)). This has enabled learning longer sequences than previously possible. This version of the Switch Transformer does not employ attention sparsity, but these techniques are complimentary, and, as future work, these could be combined to potentially improve learning on tasks requiring long contexts.
...
On a time basis, Switch Transformers can be far more efficient than dense-models with sharded parameters (Figure [6](#S3.F6 "Figure 6 ‣ 3.3 Scaling Versus a Larger Dense Model ‣ 3 Scaling Properties ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity")).
...
The motivation to try sparse models has been stymied by the massive success of scaling dense models (the success of which is partially driven by co-adaptation with deep learning hardware as argued in Hooker ([2020](#bib.bib20))). Further, sparse models have been subject to multiple issues including (1) model complexity, (2) training difficulties, and (3) communication costs.
...
5. 5.
   ...
   In Appendix [A](#A1 "Appendix A Switch for Attention ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity"), we report quality improvement adding these inside Self-Attention layers, where our layer replaces the weight matrices which produce Q, K, V.
...
Shazeer et al. ([2018](#bib.bib47)); Lepikhin et al. ([2020](#bib.bib30)) designed MoE Transformers (Shazeer et al., [2017](#bib.bib46)) by adding MoE layers into the dense feedfoward network (FFN) computations of the Transformer.
...
Table [10](#A1.T10 "Table 10 ‣ Appendix A Switch for Attention ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") records the quality after a fixed number of steps as well as training time for several variants.
...
Lepikhin et al. ([2020](#bib.bib30)) adapts a Mixture-of-Expert model and addresses expert overflow by passing its representation to the next layer without processing through a residual connection which we also follow.
...
As in reinforcement learning, a classic exploration-exploitation dilemma arises (Sutton and Barto, [2018](#bib.bib53)).
These issues have been similarly noted and addressed differently by Rosenbaum et al. ([2017](#bib.bib42)) which demonstrated success in multi-task learning.
This particular setting most closely matches that of a contextual bandit (Robbins, [1952](#bib.bib40)).
...
The resulting impact on model quality is reported in Table [11](#A3.T11 "Table 11 ‣ Appendix C Encouraging Exploration Across Experts ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity").
...
However, in the largest model regime (T5-11B/T5-XXL) our largest Switch models, as mentioned in Section [5.6](#S5.SS6 "5.6 Towards Trillion Parameter Models ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity"), do not always translate their upstream perplexity well to downstream fine-tuning on the SuperGLUE task.
...
Pseudocode for Switch Transformers in Mesh Tensorflow (Shazeer et al., [2018](#bib.bib47)). No model parallelism is being used for the below code (see [5.4](#S5.SS4 "5.4 Expert and Data Parallelism ‣ 5 Designing Models with Data, Model, and Expert-Parallelism ‣ Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") for more details).
...
* Cho and Bengio (2014)
  ...
  *arXiv preprint arXiv:1406.7362*, 2014.
* Clark et al. (2018)
...
* Gale et al. (2020)
  ...
  *arXiv preprint arXiv:2006.10901*, 2020.
* Gray et al. (2017)
...
* Hermann et al. (2015)
  ...
  pages 1693–1701. Curran Associates, Inc., 2015.
  URL
...
* Puigcerver et al. (2020)
  ...
  André Susano Pinto, Sylvain Gelly, Daniel Keysers, and Neil Houlsby.
