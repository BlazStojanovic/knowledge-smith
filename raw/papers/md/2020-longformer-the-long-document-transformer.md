...
TriviaQA. We finally introduce the Longformer-Encoder-Decoder (LED), a Longformer variant for supporting long document generative sequence-to-sequence tasks, and demonstrate its effectiveness on the arXiv summarization dataset.111<https://github.com/allenai/longformer>
...
with the vectorized Longformer-chunk being the fastest. More details are in section [3.2](#S3.SS2 "3.2 Implementation ‣ 3 Longformer ‣ Longformer: The Long-Document Transformer").

To address this limitation, we present Longformer, a modified Transformer architecture
with a self-attention operation that scales linearly with the sequence length, making it versatile for processing long documents (Fig [1](#S1.F1 "Figure 1 ‣ 1 Introduction ‣ Longformer: The Long-Document Transformer")).
...
Recent work has addressed the computational inefficiency of Transformers on long sequences (see Tab. [1](#S1.T1 "Table 1 ‣ 1 Introduction ‣ Longformer: The Long-Document Transformer")).
...
| \hdashline[0.4pt/2pt] Reformer reformer | sparse | yes | no | no |
| Sparse sparseOpenai | sparse | yes | no | no |
| Routing roy2020efficient | sparse | yes | no | no |
| \hdashline[0.4pt/2pt] BP-Transformer BPTransformer | sparse | yes | MT | no |
| Blockwise blockbert | sparse | no | QA | yes |
| \hdashline[0.4pt/2pt] Our Longformer | sparse | yes | multiple | yes |
...
Tab. [1](#S1.T1 "Table 1 ‣ 1 Introduction ‣ Longformer: The Long-Document Transformer") summarizes recent prior work on long documents. Two types of self-attention approaches have been explored.
...
Our implementation (§[3](#S3 "3 Longformer ‣ Longformer: The Long-Document Transformer")) also includes a custom CUDA kernel, but it is more flexible and maintainable than BlockSparse which is implemented in C++, and designed for a specific version of TensorFlow.
We also introduce additional task motivated global attention patterns suitable for common NLP tasks
(§[3](#S3 "3 Longformer ‣ Longformer: The Long-Document Transformer")) and show they are essential for good performance in the transfer learning setting.
...
Given a fixed window size w𝑤w, each token attends to 12​w12𝑤\frac{1}{2}w tokens on each side (Fig. [2(b)](#S1.F2.sf2 "In Figure 2 ‣ 1 Introduction ‣ Longformer: The Long-Document Transformer")).
...
Depending on the application, it might be helpful to use different values of w𝑤w for each layer to balance between efficiency and model representation capacity (§[4.1](#S4.SS1 "4.1 Attention Pattern ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer")).

##### Dilated Sliding Window

To further increase the receptive field without increasing computation, the sliding window can be “dilated”. This is analogous to dilated CNNs Oord2016WaveNetAG where the window has gaps of size dilation d𝑑d (Fig. [2(c)](#S1.F2.sf3 "In Figure 2 ‣ 1 Introduction ‣ Longformer: The Long-Document Transformer")).
...
Fig. [2(d)](#S1.F2.sf4 "In Figure 2 ‣ 1 Introduction ‣ Longformer: The Long-Document Transformer") shows an example of a sliding window attention
...
In regular transformers, attention scores are computed as in Eqn. [1](#S3.E1 "In Linear Projections for Global Attention ‣ 3.1 Attention Pattern ‣ 3 Longformer ‣ Longformer: The Long-Document Transformer").
...
of Q​KT𝑄superscript𝐾𝑇QK^{T}. As shown in Fig. [1](#S1.F1 "Figure 1 ‣ 1 Introduction ‣ Longformer: The Long-Document Transformer"), this results in a linear increase
...
Fig. [1](#S1.F1 "Figure 1 ‣ 1 Introduction ‣ Longformer: The Long-Document Transformer") compares the performance of three different ways of implementing it:
...
Tab. [4.2.1](#S4.SS2.SSS1 "4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer") and [3](#S4.T3 "Table 3 ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer") summarize evaluation results on text8 and enwik8 datasets.
...
Tab. [3](#S4.T3 "Table 3 ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer") shows that Longformer outperforms the comparable Transformer-XL model,
...
§[2](#S2 "2 Related Work ‣ Longformer: The Long-Document Transformer").
...
| \hdashline[0.2pt/0.2pt] Dataset enwik8 |  |  |  |
...
The bottom of Tab. [4](#S4.T4 "Table 4 ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer") shows the impact of adding dilation.
...
We pretrain Longformer using fairseq ott2019fairseq on a corpus of long documents that we compiled (see Appendix [C](#A3 "Appendix C Pretraining Data ‣ Tensor Virtual Machine (TVM) ‣ 6.4 Results ‣ 6 Tasks ‣ Frozen RoBERTa Weights ‣ 5 Pretraining and Finetuning ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer") for corpus details).
...
Tab. [5](#S5.T5 "Table 5 ‣ Position Embeddings ‣ 5 Pretraining and Finetuning ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer") shows the BPC on the development
...
The Longformer variant replaces the RoBERTa self-attention mechanism with our windowed attention used during pretraining, plus a task motivated global attention. The global attention uses additional linear projections (§[3.1](#S3.SS1 "3.1 Attention Pattern ‣ 3 Longformer ‣ Longformer: The Long-Document Transformer")).
...
See Appendix [D](#A4 "Appendix D Task specific model details ‣ Tensor Virtual Machine (TVM) ‣ 6.4 Results ‣ 6 Tasks ‣ Frozen RoBERTa Weights ‣ 5 Pretraining and Finetuning ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer") for further details about the models and hyperparameters.
...
We also evaluate the performance of Longformer-large on long context QA tasks. Tab. [8](#S6.T8 "Table 8 ‣ Longformer-large for QA ‣ 6.4 Results ‣ 6 Tasks ‣ Frozen RoBERTa Weights ‣ 5 Pretraining and Finetuning ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer")
...
code111111<https://github.com/kimiyoung/transformer-xl>
...
We employed mixed precision training (floating points 16 and 32) using apex121212<https://github.com/NVIDIA/apex> to reduce memory consumption and speed-up training. However, we kept the attention computation in fp32
...
All hyperparameters and stage configurations are listed in Tab. [12](#A2.T12 "Table 12 ‣ Appendix B Character LM Hyperparameters ‣ Tensor Virtual Machine (TVM) ‣ 6.4 Results ‣ 6 Tasks ‣ Frozen RoBERTa Weights ‣ 5 Pretraining and Finetuning ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer").
...
13 days.
Most of our hyperparameter search is similar to the ablation in Tab. [4](#S4.T4 "Table 4 ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer") where we run the configuration for 150K steps
on text8.
...
reported in Tab. [12](#A2.T12 "Table 12 ‣ Appendix B Character LM Hyperparameters ‣ Tensor Virtual Machine (TVM) ‣ 6.4 Results ‣ 6 Tasks ‣ Frozen RoBERTa Weights ‣ 5 Pretraining and Finetuning ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer") is determined by running each phase until the validation BPC stops getting better.

|  |  |
| --- | --- |
| Param | Value |
| Position Embeddings | Relative and Sinusoidal as in transformerxl |
| Small model config | 12 layers, 8 heads, 512 hidden size as in transformerxl |
| Large model config | 30 layers, 8 heads, 512 hidden size as in sparseOpenai |
| Optimizer | AdamW |
| Dropout | 0.2 (small model), 0.4 (large model) |
| Gradient clipping | 0.25 |
| Weight Decay | 0.01 |
| Layernorm Location | pre-layernorm layernorm |
| Activation | GeLU |
| Number of phases | 5 |
...
Hyperparameters of the best configuration are listed in Tab. [14](#A4.T14 "Table 14 ‣ HotpotQA ‣ Appendix D Task specific model details ‣ Tensor Virtual Machine (TVM) ‣ 6.4 Results ‣ 6 Tasks ‣ Frozen RoBERTa Weights ‣ 5 Pretraining and Finetuning ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer"). All other hyperparameters are similar to RoBERTa’s. For hyperparameter search, we only tuned LR for the RoBERTa
...
We trained the model using Adam optimizer with linear warmup (1000 steps) and linear decay. We used minimal hyperparameter tuning using LRs of 3e-5 and 5e-5 and epochs of 3 to 7 and found the model with LR of 3e-5 and 5 epochs to work best. We conduct the same hyperparameter search for the RoBERTa baseline as well. The rest of hyperparameters are reported in Tab [14](#A4.T14 "Table 14 ‣ HotpotQA ‣ Appendix D Task specific model details ‣ Tensor Virtual Machine (TVM) ‣ 6.4 Results ‣ 6 Tasks ‣ Frozen RoBERTa Weights ‣ 5 Pretraining and Finetuning ‣ 4.2.2 Ablation Study ‣ 4.2.1 Results ‣ 4.2 Experiment Setup ‣ 4 Autoregressive Language Modeling ‣ Longformer: The Long-Document Transformer").
